...
const EscuderiasPage = () => {
  // Datos de prueba para las tarjetas de escuderías, con colores y slugs
  // Asegúrate de tener estas imágenes de largeLogo en public/imagenes/
    ...
    <div className="escuderias-page-container">
      ...
      <div className="escuderias-grid">
        {placeholderTeams.map(team => {
          console.log(`Attempting to load large logo for ${team.name}: ${team.largeLogo}`);
          return (
            <Link key={team.id} to={`/escuderias/${team.slug}`}>
              <a className="escuderia-card"
                 style={{
                   backgroundColor: team.color, // Color base de la escudería
                   // Los degradados se mantienen aquí, el logo va en el overlay
                   backgroundImage: `
                   ...
                   `,
                   backgroundRepeat: 'no-repeat, no-repeat', // Solo dos capas para los degradados
                   backgroundPosition: 'left top, left top', // Posición para cada capa
                   backgroundSize: '100% 100%, 100% 100%', // Tamaño para cada capa
                 }}
              >
                {/* NUEVO: Div para el logo de fondo difuminado */}
                <div
                  className="background-logo-overlay"
                  style={{
                    backgroundImage: `url(${team.largeLogo})`,
                  }}
                ></div>
                ...
                <img src={team.carImage || 'https://placehold.co/250x80/cccccc/000000?text=CAR'} alt={`${team.name} Car`} className="car-image" />
              </a>
            </Link>
          );
        })}
